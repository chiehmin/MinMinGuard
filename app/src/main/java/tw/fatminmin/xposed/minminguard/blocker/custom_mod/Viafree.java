...
public final class Viafree
{
    ...
    public static boolean handleLoadPackage(final String packageName, XC_LoadPackage.LoadPackageParam lpparam, final boolean removeAd)
    {
        if (!lpparam.packageName.contains(pkg))
            return false;

        XposedHelpers.findAndHookMethod(className, lpparam.classLoader, method,
                new XC_MethodHook()
                {
                    @Override
